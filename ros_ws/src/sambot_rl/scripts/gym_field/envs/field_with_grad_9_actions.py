...
class SpatialTemporalFieldWithGradConcNineActions(gym.Env):
    """
    This environment is similar to the regular field, except the state vector contains the gradient
    and concentration values as well, and has 5 actions, one extra for staying still
    """
    ...
    def __init__(
        ...
        super(SpatialTemporalFieldWithGradConcNineActions, self).__init__()
        ...
        low = np.array([0.0, 0.0, 0.0, -100.0, -100.0])
        high = np.array([99.0, 99.0, 25.0, 100.0, 100.0])
    ...
    def step(self, action_id):
        ...
        assert action in self.actions, "%s (%s) invalid" % (action, type(action))
        ...
        if (hit_wall):
            ...
            return (reward, [self.agent_position[0], self.agent_position[1], concentration, self.agent_gradients[0], self.agent_gradients[1]], True, {})
        ...
        reward = self.calculate_reward_2(
            next_position, frac_coverage_improvement)
        ...
        return (reward, [next_position[0], next_position[1], concentration, self.agent_gradients[0], self.agent_gradients[1]], done, observations)
    ...
    def reset(self):
        ...
        return [self.agent_position[0], self.agent_position[1], concentration, self.agent_gradients[0], self.agent_gradients[1]]
    ...
    def calculate_reward_2(self, next_state, frac_coverage_improvement):
        ...
        #     reward = -1000
        # return reward
        # return (1e-2 * np.sum(self.curr_view_scope)) + (0.1 * float(frac_coverage_improvement))
