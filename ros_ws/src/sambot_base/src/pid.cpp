...
namespace sambot_base
{
    ...
    double PID::operator()(const double &measured_value, const double &setpoint, const ros::Duration &dt)
    {
        ...
        // ROS_INFO_STREAM("PID computed command: =======>" << output << "=== error ===" << error_);
    ...
    void PID::updateParameters(double f, double p, double i, double d) {
    	Gains gains = getGains();
    	setGains(p, i, d, gains.i_max_, gains.i_min_, gains.antiwindup_);
    	f_ = f;
	ROS_INFO_STREAM("Update PID Gains: F=" << f << ", P=" << p << ", I=" << i << ", D=" << d);
    ...
    void PID::updateDynamicReconfig()
    {
        ...
        // Get starting values
        sambot_base::ParametersConfig config;
        config.f = f_;
    ...
    void PID::updateDynamicReconfig(sambot_base::ParametersConfig config)
    {
        // Make sure dynamic reconfigure is initialized
        if(!dynamic_reconfig_initialized_)
            return;
    ...
    void PID::dynamicReconfigCallback(sambot_base::ParametersConfig &config, uint32_t /*level*/)
    {
